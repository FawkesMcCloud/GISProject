from Utils import filter_files_paths, create_preview
from models.Split import split_in_chunks
from models.Mask import *
from models.modis.Minimise import minimize
from models.modis.HDF4toHDF5 import convert_hdf
import os
...
def diplom():
    # Setup folders
    original_modis_files_dir = os.path.normpath("D:\\college\\Data\\Diplom\\MOD_ORIG")
    hdf5_output = os.path.normpath("D:\\College\\Data\\Diplom\\MOD_HDF5")
    min_output = os.path.normpath("D:\\College\\Data\\Diplom\\DATASET")
    chunks_output = os.path.normpath("D:\\College\\Data\\Diplom\\DATASET\\Chunks")
    ...
    # Save only data that is used for learning
...
def previews():
    ...
    for i, f in enumerate(files):
        image = create_preview(f, "clouds", "clouds", "clouds")
        imageC = create_preview(f, "cloud2", "cloud2", "cloud2")
        imageS = create_preview(f, "snow", "snow", "snow")
        image.save(f[:-3] + "_C.png", format="png", optimize=True)
        imageS.save(f[:-3] + "_S.png", format="png", optimize=True)
        imageC.save(f[:-3] + "_C2.png", format="png", optimize=True)
